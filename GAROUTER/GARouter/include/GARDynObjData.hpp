...
#ifndef GARDYNOBJDATA_HPP_
#define GARDYNOBJDATA_HPP_
...
public:
	 ...
	 * Empty constructor.
	 */
	GARDynObjData() = delete;

	/**
	 * Parameterized constructor.
	 * @param logger	Reference to the ga-router logger.
	 */
	GARDynObjData(CLogger* logger);

	/**
	 * Default virtual constructor.
	 */
	virtual ~GARDynObjData() = default;
	...
	/**
	 * Get the offset time for the (scaled) flow times.
	 * @return	The offset time for the (scaled) flow times.
	 */
	SUMOTime getOffset(void) const;
	...
	/**
	 * Get a reference to the routes solution from the static approach.
	 * @return	The set of routes which is the solution
	 * 			from the static GA.
	 */
	const std::vector<std::string>& getRoutesSolution(void) const;
	...
	/**
	 * Set the offset time for the (scaled) flow times.
	 * @param offset  The offset time for the (scaled) flow times.
	 */
	...
	/**
	 * Set the routes solution from the static approach.
	 * @param genomeSolution  The set of routes which is the solution
	 * 						  from the static GA.
	 */
	void setRoutesSolution(const gar::genome_static& genomeSolution);
...
private:
	...
	//! The second to substract from (scaled) flow times
	SUMOTime offset;
	...
	//! The solution from the static GA. It contains the traffic flow routes.
	std::vector<std::string> routesSolution;
