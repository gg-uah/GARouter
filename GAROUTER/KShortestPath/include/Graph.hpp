...
protected: // members
	...
	map<std::string, double> m_mpEdgeCodeWeight;
	mutable vector<BaseVertex*> m_vtVertices;
	int m_nEdgeNum;
	int m_nVertexNum;

	mutable map<std::string, BaseVertex*> m_mpVertexIndex;

	// Members for graph modification
	set<std::string> m_stRemovedVertexIds;
	set< pair<std::string, std::string> > m_stRemovedEdge;
...
public:
	...
	void add_link(const std::string& from,
				  const std::string& to,
				  const double& weight );
	void setv();

	BaseVertex* get_vertex(const std::string& node_id) const;
	
	std::string get_edge_code(const BaseVertex* start_vertex_pt, const BaseVertex* end_vertex_pt) const;
	...
	void remove_edge(const pair<std::string,std::string> edge)
	{
		m_stRemovedEdge.insert(edge);
	}

	void remove_vertex(const std::string vertex_id)
	{
	...
	void recover_removed_edge(const pair<std::string,std::string> edge)
	{
		m_stRemovedEdge.erase(m_stRemovedEdge.find(edge));
	}

	void recover_removed_vertex(std::string vertex_id)
	{
