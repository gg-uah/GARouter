...
void Graph::add_link(const std::string& start,
					 const std::string& end,
					 const double& weight) {
	///3.2.1 construct the vertices
	BaseVertex* start_vertex_pt = get_vertex(start);
	BaseVertex* end_vertex_pt = get_vertex(end);
...
BaseVertex* Graph::get_vertex(const std::string& node_id ) const {
	if (m_stRemovedVertexIds.find(node_id) != m_stRemovedVertexIds.end()) {
		return NULL;
	} else {
		BaseVertex* vertex_pt = NULL;
		const map<std::string, BaseVertex*>::iterator pos = m_mpVertexIndex.find(node_id);
...
std::string Graph::get_edge_code(const BaseVertex* start_vertex_pt, const BaseVertex* end_vertex_pt) const
{
	...
	return start_vertex_pt->getID() + "to" + end_vertex_pt->getID();
}
...
double Graph::get_edge_weight( const BaseVertex* source, const BaseVertex* sink ) {
	std::string source_id = source->getID();
	std::string sink_id = sink->getID();
...
void Graph::get_adjacent_vertices( BaseVertex* vertex, set<BaseVertex*>& vertex_set ) {
	std::string starting_vt_id = vertex->getID();
	...
	if (m_stRemovedVertexIds.find(starting_vt_id) == m_stRemovedVertexIds.end()) {
		...
		for (set<BaseVertex*>::const_iterator pos = (*vertex_pt_set).begin(); pos != (*vertex_pt_set).end(); ++pos) {
			std::string ending_vt_id = (*pos)->getID();
...
void Graph::get_precedent_vertices(BaseVertex* vertex, set<BaseVertex*>& vertex_set) {
	if (m_stRemovedVertexIds.find(vertex->getID()) == m_stRemovedVertexIds.end()) {
		std::string ending_vt_id = vertex->getID();
		...
		{
			std::string starting_vt_id = (*pos)->getID();
			if (m_stRemovedVertexIds.find(starting_vt_id) != m_stRemovedVertexIds.end()
				|| m_stRemovedEdge.find(make_pair(starting_vt_id, ending_vt_id)) != m_stRemovedEdge.end()) {
				continue;
			}
...
double Graph::get_original_edge_weight(const BaseVertex* source, const BaseVertex* sink) {
	map<std::string, double>::const_iterator pos = m_mpEdgeCodeWeight.find(get_edge_code(source, sink));
